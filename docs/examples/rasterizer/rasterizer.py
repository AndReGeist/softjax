...
def load_obj(path, verbose=True):
    """Minimal OBJ loader. Triangulates polygons via fan and returns
    (vertices, normals, tex_coords) as ``(n_tris, 3, ...)`` arrays.

    Assumes the OBJ convention of CCW front-facing polygons; ``verbose``
    silences the progress notes (used by the notebook)."""
    ...
    if n_flipped and verbose:
        print(f"  load_obj: flipped {n_flipped}/{n_tris} CW triangles to CCW.")
...
def normal_shader(pixel_xy, tex_coord, normal, depth):
...
def white_shader(pixel_xy, tex_coord, normal, depth):
...
def main():
    ...
    vertices, normals, tex_coords = load_obj(
        os.path.join(here, "meshes", MODEL_NAME + ".obj"))
    ...
    if MODEL_NAME == "dave":
        ...
        rot = rotation_x(jnp.deg2rad(180))
        scale = 1.3
    ...
    #vertices2, normals2, tex_coords2 = load_obj(os.path.join(here, "cube_fine.obj"))
    ...
    def make_scene(angle_rad):
        model = Model(
            ...
            transform=Transform(
                position=pos,
                rotation=rotation_y(angle_rad) @ rot,
                scale=scale,
            ),
